...
void __sthread_finish(void) {
    ...
    // we lock so this will never unlock and keep running at any point
    __sthread_lock();
    __sthread_switch();
}
 ...
 */
void sthread_yield() {
    // we lock the thread so there's only one running
    // when we yield, we want to switch to another one so this one locks
    __sthread_lock();
    __sthread_switch();
}
...
void sthread_block() {
    ...
    // need to lock before we call switch so that only on thread is executing
    // blocked threads should not be running so need to lock
    __sthread_lock();
    __sthread_switch();
}
...
void sthread_unblock(Thread *threadp) {
    ...
    // lock thread so the operation is done atomically
    __sthread_lock();
    ...
    // we need to unlock so it will become ready again and usable
    __sthread_unlock();
}
