...
ThreadContext *__sthread_scheduler(ThreadContext *context) {
    ...
    // otherwise enqueue unless blocked
    ...
    if (current == NULL) {
        // checked for blocked threads
        if (blocked_queue.head == NULL) {
            fprintf(stderr, "All threads have completed successfully!\n");
            exit(0);
        }
        else {
            fprintf(stderr, "The program has become deadlocked!\n");
            exit(1);
        }
    }
    else {
...
Thread * sthread_create(void (*f)(void *arg), void *arg) {
    ...
    char *stackstart = (char *)malloc(DEFAULT_STACKSIZE);
    ...
    // check for failure
    if (!newthread) {
        fprintf(stderr, "ERROR:  Couldn't allocate new thread!\n");
        abort();
    }

    // initialization
    newthread->state = ThreadReady;
    newthread->memory = stackstart;
    newthread->context = __sthread_initialize_context(\
        stackstart + DEFAULT_STACKSIZE, f, arg);

    // add to ready queue
    queue_append(&ready_queue, newthread);
